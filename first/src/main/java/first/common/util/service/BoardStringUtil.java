/**
 * @Class Name  : KctcStringUtil.java
...
public class BoardStringUtil {
    ...
    /**
     * <code>subject</code>에 포함된 각각의 문자를 object로 각각 변환한다.
     * 
     * @param source 원본 문자열
     * @param subject 원본 문자열에 포함된 특정 문자열
     * @param object 변환할 문자열
	...
	try {
		...
		// 시작문자 및 종료문자 중에서 랜덤 숫자를 발생시킨다.
	    	...
	    	if(data<Integer.MAX_VALUE){
	    		...
	    		Logger.getLogger(BoardStringUtil.class).warn("data value is too large to perform addition.");
	    	}
		...
		Logger.getLogger(BoardStringUtil.class).warn("getRandomStr Exception.     " + e);
	}
...
/**
	...
	try {
		...
		Logger.getLogger(BoardStringUtil.class).warn("getSpclStrCnvr Exception.     " + e);
	}catch (Exception e) {
		Logger.getLogger(BoardStringUtil.class).warn("getSpclStrCnvr Exception.     " + e);
	}
	...
	try {
		...
		Logger.getLogger(BoardStringUtil.class).warn("getTimeStamp Exception.     " + e);
	}catch (Exception e) {
		Logger.getLogger(BoardStringUtil.class).warn("getTimeStamp Exception.     " + e);
	}
    ...
    public static String getHtmlStrCnvr(String srcString) {
		...
		try 
		{
			...
			Logger.getLogger(BoardStringUtil.class).warn("getHtmlStrCnvr Exception.     " + e);
		}catch (Exception ex)
		{
			Logger.getLogger(BoardStringUtil.class).warn("getHtmlStrCnvr Exception.     " + ex);
		}
	...
	/**
	  * VO와 매핑하여 unscript 처리
	  * @author 이진
	  * @since 2013. 11. 22.
	  * @param voObj unscript로 설정할 vo명
	  * @return List<ExtrnlUserAccRqstInfoVO> 외부사용자계정요청정보
	*/	
	public static void unscriptVoVal(Object voObj) throws Exception{
		String fieldName = "";
		String firstChar = "";
		
		Field[] arrUserRqstField = voObj.getClass().getDeclaredFields();		
		DirectFieldAccessor myObjectAccessor = new DirectFieldAccessor(voObj);

		for (int i=1; i<arrUserRqstField.length; i++) {
			fieldName = arrUserRqstField[i].getName();
			firstChar = fieldName.substring(0, 1).toUpperCase();
			Method getMethod = voObj.getClass().getMethod("get" + firstChar + fieldName.substring(1));
			Object result = getMethod.invoke(voObj);
			if(result != null) {
				myObjectAccessor.setPropertyValue(fieldName, unscript(String.valueOf(result)));
			}
		}
